...
| Idle timeout | **15 minutes of no activity closes the lobby — including active matches.** Any client action resets the timer. |
...
- A user can be in **at most one lobby at a time** (second tab / second join → error "You're already in a lobby").
...
- The 15-minute threshold is exposed as `PLAY_IDLE_TIMEOUT_MS` env var (default `900000`) so tests can shrink it.
      ...
      • elsewhere + active session:     upgrade handler: verify JWT before accepting
        floating "return to match" pill
                                        ...
                                        LobbyManager (in-memory Map<code, Lobby>)
                                         ...
                                         ├─ idle sweep (30s interval, 15 min threshold)
...
|---------|---------|-------|
| `create_lobby` | `{ deckId }` | Deck must belong to user and have ≥ 1 card. Snapshot the deck. Reject if user is already in a lobby. Server generates the code and replies with `lobby_joined`. |
| `join_lobby` | `{ code, deckId? }` | `deckId` required when joining a *waiting* lobby (validated as above). If the match is *active*: only allowed to **rejoin your existing seat** (`deckId` ignored); new joiners get error "Match already in progress". |
...
| `move_card` | `{ instanceId, from: 'hand'\|'table'\|'graveyard', to: 'hand'\|'deck'\|'graveyard'\|'table' }` | Card must exist in the requester's `from` zone (this is what enforces "you can't take cards from another player's graveyard"). `to: 'deck'` inserts at index 0 (top). `from === to` is ignored. Stale/unknown instance → `error`. |
...
Define `CardInstance`, `PlayerMatchState`, `MatchState`, `LobbyInfo`, `LobbyPlayerInfo`, and the client→server / server→client message unions exactly as in §4. Export a `PLAY_IDLE_TIMEOUT_MS` constant reading `process.env.PLAY_IDLE_TIMEOUT_MS ?? 900_000`.
...
Rules: unique code generation (retry on collision); max 4 seats; one lobby per user; deck ownership + non-empty check at join (snapshot stored on the seat); waiting-join of a full lobby → error "Lobby is full"; host leaving while waiting → promote `players[0]` of remaining, or close with `session_end { reason: 'lobby_empty' }`; if all seats become disconnected while waiting → close immediately.
After every change, broadcast `lobby_update` (or `lobby_joined` to the joiner) to all connected members.

**Step 2.2 — Idle sweep.**
A single `setInterval(30_000)` started in `attachPlay`: for each lobby, if `Date.now() - lastActivityAt > PLAY_IDLE_TIMEOUT_MS`, close it: send `session_end { reason: 'idle_timeout', message: 'Lobby closed after 15 minutes of inactivity' }` to all connected members, then delete from the map. The same sweep covers waiting lobbies and active matches (per decision §1).
...
| User already in a lobby, creates/joins another | `error` "You're already in a lobby". |
...
| 15 min idle, waiting or active | `session_end { idle_timeout }`; lobby deleted. Any action resets the timer. |
...
| Same user opens a second tab in a lobby | Second tab's create/join is rejected; it can still spectate nothing — it simply isn't in the lobby (acceptable v1 behavior). |
| Malformed WS message / bad JSON | `error` sent, socket stays open. |

## 7. Manual QA checklist

Mock mode: select deck → Play Mock → commander on table, empty hand → click deck to draw several cards → right-click deck → Shuffle → drag hand card to table → drag it back to hand / to graveyard / to top of deck (verify via next draw) → open own graveyard list → drag a row back to table → exit.

Multiplayer: host in tab A (note code) → join in tab B with code → both see each other in LobbyModal → start → verify shuffled decks + commander placement on both sides → A draws, B shuffles, A drags hand→table → B sees updates live → mid-match, tab A clicks My Decks and hits browser back → match keeps running with no state loss, floating "return to match" pill appears → click the pill → session resumes exactly as left → B opens A's graveyard (empty at first), then after A moves a card there: list shows it, rows not draggable → B tries dragging one of A's table cards (impossible — no drag handle) → close tab B (disconnect) → A sees gray "disconnected" dot, cards intact → reopen tab B, re-enter code → seat restored, state resynced → let the lobby sit 15 min (or use the env override) → both land on Play landing with the idle banner.
